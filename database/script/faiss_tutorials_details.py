...
def build_embedding_text(index: str, dir_structure: str, detailed_tutorial: str, max_chars: int = 2500) -> str:
    """Build searchable text for tutorial-detail retrieval.

    The metadata still keeps the full tutorial, but the embedding text must include
    more than case name and directory structure. Otherwise queries mentioning files
    like constitutiveProperties, fvSolution, tau, theta, or model-specific
    keywords cannot retrieve the right detail page reliably.
    """
    file_names = "\n".join(re.findall(r"<file_begin>file name:\s*(.*)", detailed_tutorial))
    plain_tutorial = re.sub(r"</?[^>]+>", " ", detailed_tutorial)
    plain_tutorial = re.sub(r"\s+", " ", plain_tutorial).strip()
    if len(plain_tutorial) > max_chars:
        plain_tutorial = plain_tutorial[:max_chars]
    return tokenize("\n".join([index, dir_structure, file_names, plain_tutorial]))
   ...
   # Step 1: Parse command-line arguments
    ...
    for match in matches:
        ...
        documents.append(Document(
            page_content=build_embedding_text(index, dir_structure, detailed_tutorial),
            metadata={
