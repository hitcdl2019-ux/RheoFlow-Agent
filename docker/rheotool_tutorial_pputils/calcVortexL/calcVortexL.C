...
Foam::ppUtils::calcVortexL::calcVortexL
(
    const word& name,
...
:
ppUtil(name, dict, mesh), 
vortTop_(mesh.boundaryMesh().findPatchID("wall_vorttop")), 
vortDown_(mesh.boundaryMesh().findPatchID("wall_vortdown")), 
lipTop_(mesh.boundaryMesh().findPatchID("wall_liptop")),
lipDown_(mesh.boundaryMesh().findPatchID("wall_lipdown")) 
{
...
void Foam::ppUtils::calcVortexL::createFile()
{
...
void Foam::ppUtils::calcVortexL::update()
{
if (enabled_)
 {
   counter_++;
...
//****  User-defined function STARTS here *****//
           ...
           // col 0: Time
           outS_()  << mesh().time().value() << tab; 
           outS1_() << mesh().time().value() << tab; 
           outS2_() << mesh().time().value() << tab; 
           outS3_() << mesh().time().value() << tab; 

           const volVectorField& C = mesh().C();
           
// Xr_top
        {
           const polyPatch& cPatchvort = mesh().boundaryMesh()[vortTop_];
...
// Xr_down
        {
          const polyPatch& cPatchvort = mesh().boundaryMesh()[vortDown_];
...
// Xl_top
        {
          const polyPatch& cPatchvort = mesh().boundaryMesh()[lipTop_];
...
// Xl_down
        {
          const polyPatch& cPatchvort = mesh().boundaryMesh()[lipDown_];
