...
shaft_(mesh.boundaryMesh().findPatchID("shaft")),
piston_(mesh.boundaryMesh().findPatchID("piston"))
{
...
//****  User-defined function STARTS here *****//
	   ...
	   volSymmTensorField F = symm( L + L.T() ) * eta_ - p * symmTensor::I * rho_;
           ...
           vector Fpatch =  gSum( ( -mesh().boundaryMesh()[piston_].faceAreas() ) & F.boundaryField()[piston_] )
                          + gSum( ( -mesh().boundaryMesh()[shaft_].faceAreas() ) & F.boundaryField()[shaft_] );
