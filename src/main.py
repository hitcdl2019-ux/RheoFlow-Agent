...
from services.intake import allow_unstamped_override, validate_receipt
import json
...
def initialize_state(user_requirement: str, config: Config, custom_mesh_path: Optional[str] = None) -> GraphState:
    ...
    state = GraphState(
        ...
        tutorial_template_match=None,
        tutorial_template_import=None,
        mesh_info=None,
        mesh_commands=None,
...
def enforce_intake_receipt(prompt_path: str) -> dict | None:
    if allow_unstamped_override():
        print('<intake_override unstamped_override="true">FOAMAGENT_DEV_ALLOW_UNSTAMPED is set; skipping intake receipt validation.</intake_override>')
        return {"unstamped_override": True}
    receipt = validate_receipt(prompt_path)
    print(f'<intake_receipt status="ready" path="{prompt_path}.intake.json"/>')
    return receipt
...
def main(user_requirement: str, config: Config, custom_mesh_path: Optional[str] = None):
    ...
    try:
        ...
        elif termination_reason == "openfoam_environment_blocked":
            print("<workflow_end>Workflow stopped because the OpenFOAM execution environment is blocked.</workflow_end>")
        else:
...
if __name__ == "__main__":
    ...
    enforce_intake_receipt(args.prompt_path)
