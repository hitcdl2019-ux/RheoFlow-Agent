...
from services.case_manifest import create_case_manifest
...
def planner_node(state):
    ...
    case_target = plan_data["case_target"]
    ...
    solver_route = plan_data.get("solver_route")
    benchmark_match = plan_data.get("benchmark_match")
    tutorial_template_match = plan_data.get("tutorial_template_match")
    ...
    create_case_manifest(
        case_dir,
        case_target,
        problem_intent=plan_data.get("problem_intent"),
        physics_spec=plan_data.get("physics_spec"),
        rheology_spec=plan_data.get("rheology_spec"),
    ...
    if solver_route:
        print(f"<solver_route>{solver_route}</solver_route>")
    if benchmark_match:
        print(f"<certified_benchmark>{benchmark_match}</certified_benchmark>")
    if tutorial_template_match:
        ...
        print(f"<{tag}>{tutorial_template_match}</{tag}>")
    ...
    if benchmark_match or tutorial_template_match:
        ...
        if benchmark_match:
            print("<mesh_type>standard_mesh - Certified benchmark import.</mesh_type>")
        elif tutorial_template_match.get("template_kind") == "foundation":
            print("<mesh_type>standard_mesh - Foundation tutorial template import.</mesh_type>")
        else:
            print("<mesh_type>standard_mesh - rheoTool tutorial template import.</mesh_type>")
    else:
        if state.get("llm_service") is None:
