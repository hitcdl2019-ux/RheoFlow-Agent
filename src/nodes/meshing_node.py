...
def meshing_node(state):
    ...
    if case_target is None:
        raise ValueError("CaseTarget is missing; refusing to mesh without an explicit OpenFOAM channel")
    ...
    if mesh_type == "custom_mesh":
        ...
        result = copy_custom_mesh(state.get("custom_mesh_path"), user_requirement, case_dir, case_target.channel)  # service
    elif mesh_type == "gmsh_mesh":
        print("<mesh_routing>GMSH mesh requested.</mesh_routing>")
        result = service_handle_gmsh_mesh(user_requirement, case_dir, channel=case_target.channel)  # service
    else:
