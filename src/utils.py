...
def load_faiss_dbs(config: Optional[Config] = None):
    ...
    print(f"Loading channel-isolated FAISS indices with model: {cfg.embedding_model}")
    ...
    indices = [
        ...
        "rheotool_manual_rules",
        "certified_benchmarks",
    ]

    for channel in ("v9-rheotool", "v10-foundation"):
        for index in indices:
            index_path = base_dir / channel / "faiss" / model_dir_name / index
            cache_key = f"{channel}/{index}"
            if index_path.exists():
                try:
                    dbs[cache_key] = FAISS.load_local(
                        str(index_path), embedding_model, allow_dangerous_deserialization=True
                    )
                except Exception as e:
                    print(f"Failed to load index {cache_key}: {e}")

    return dbs


# Default DB cache. Loaded lazily by retrieve_faiss so local-only modules
# such as services.run_local can be imported without loading embedding models.
FAISS_DB_CACHE = {}


def ensure_faiss_dbs_loaded(config: Optional[Config] = None) -> dict:
    global FAISS_DB_CACHE
    if not FAISS_DB_CACHE:
        FAISS_DB_CACHE = load_faiss_dbs(config)
    return FAISS_DB_CACHE
...
class LLMService:
    ...
    def __init__(self, config: object):
        ...
        elif self.model_provider.lower() == "codebuddy":
            api_key = os.getenv("CODEBUDDY_API_KEY")
            base_url = os.getenv("CODEBUDDY_BASE_URL")
            model = (
                os.getenv("FOAMAGENT_MODEL_VERSION")
                or os.getenv("CODEBUDDY_MODEL")
                or self.model_version
            )
            if not api_key:
                raise ValueError("CODEBUDDY_API_KEY is required for FOAMAGENT_MODEL_PROVIDER=codebuddy")
            if not base_url:
                raise ValueError("CODEBUDDY_BASE_URL is required for FOAMAGENT_MODEL_PROVIDER=codebuddy")
        ...
        elif self.model_provider.lower() == "novaapi":
            from langchain_openai import ChatOpenAI
            api_key = os.getenv("NOVA_API_KEY")
            base_url = os.getenv("NOVA_API_BASE_URL", "https://api.novaapis.com/v1")
            explicit_model = os.getenv("FOAMAGENT_MODEL_VERSION") or os.getenv("CODEBUDDY_MODEL")
            model = explicit_model or self.model_version
            if not api_key:
                raise ValueError("NOVA_API_KEY is required for FOAMAGENT_MODEL_PROVIDER=novaapi")
            if not explicit_model and model == "gpt-5.3-codex":
                raise ValueError(
                    "FOAMAGENT_MODEL_VERSION is required for FOAMAGENT_MODEL_PROVIDER=novaapi; "
                    "refusing to use the openai-codex default model gpt-5.3-codex"
                )
            self.model_version = model
            self.llm = ChatOpenAI(
                model=model,
    ...
    def _requires_json_prompt_structured_output(self) -> bool:
        provider = self.model_provider.lower()
        model = str(self.model_version or "").lower()
    ...
    def _is_throttling_error(self, error: Exception) -> bool:
        ...
        non_retryable_config_indicators = (
            "model_not_found",
            "model not found",
            "无可用渠道",
        ...
        if any(indicator in lowered_error for indicator in non_retryable_config_indicators):
            return False
        
        retryable_status_indicators = (
            'Error code: 429' in error_str,
            'Error code: 500' in error_str,
            'Error code: 502' in error_str,
            'Error code: 503' in error_str,
            'Error code: 504' in error_str,
            'HTTP 429' in error_str,
            'HTTP 500' in error_str,
            'HTTP 502' in error_str,
            'HTTP 503' in error_str,
            'HTTP 504' in error_str,
            'system_cpu_overloaded' in error_str,
            'system cpu overloaded' in lowered_error,
            'bad gateway' in lowered_error,
            'service unavailable' in lowered_error,
            'gateway timeout' in lowered_error,
            'temporarily unavailable' in lowered_error,
        )

        throttling_indicators = (
            error_type == 'ThrottlingException',
            error_type in {'RateLimitError', 'InternalServerError', 'APIStatusError'},
    ...
    def invoke(self,
        ...
        while True:
            try:
                if pydantic_obj:
                    if self._requires_json_prompt_structured_output():
...
class GraphState(TypedDict):
    ...
    case_target: Optional[Any]
    channel: Optional[str]
    version: Optional[str]
    distribution: Optional[str]
    solver_route: Optional[dict]
    problem_intent: Optional[Any]
    physics_spec: Optional[Any]
    rheology_spec: Optional[Any]
    workflow_plan: Optional[Any]
    benchmark_match: Optional[dict]
    benchmark_import: Optional[dict]
    tutorial_template_match: Optional[dict]
    tutorial_template_import: Optional[dict]
...
def remove_processor_folders(case_dir: str) -> None:
    """Remove stale OpenFOAM processor decomposition directories before reruns."""
    for item in os.listdir(case_dir):
        item_path = os.path.join(case_dir, item)
        if os.path.isdir(item_path) and re.fullmatch(r"processor\d+", item):
            try:
                shutil.rmtree(item_path)
                print(f"Removed processor folder: {item_path}")
            except Exception as e:
                print(f"Error removing processor folder {item_path}: {str(e)}")
...
def openfoam_command_args(channel: str, *command: str) -> List[str]:
    """Build a command that runs in a clean, channel-specific OpenFOAM shell."""
    if channel not in {"v9-rheotool", "v10-foundation"}:
        raise ValueError(f"Unsupported OpenFOAM channel: {channel!r}")
    runner = Path(__file__).resolve().parent.parent / "scripts" / "run_openfoam_channel.sh"
    if not runner.is_file():
        raise FileNotFoundError(f"OpenFOAM channel runner not found: {runner}")
    return [str(runner), channel, *map(str, command)]


def _configured_openfoam_run_user() -> str:
    return os.getenv("FOAMAGENT_OPENFOAM_RUN_USER", "openfoam").strip()


def _should_drop_openfoam_root_privileges(channel: str) -> bool:
    if channel not in {"v9-rheotool", "v10-foundation"}:
        return False
    if os.getenv("FOAMAGENT_DISABLE_OPENFOAM_NONROOT", "").strip().lower() in {"1", "true", "yes"}:
        return False
    return hasattr(os, "geteuid") and os.geteuid() == 0 and _configured_openfoam_run_user() not in {"", "root"}


def _lookup_openfoam_run_user(user: str) -> tuple[int, int]:
    import pwd
    ...
    try:
        ...
        raise RuntimeError(
            f"FOAMAGENT_OPENFOAM_RUN_USER={user!r} does not exist. "
            "OpenFOAM must not run as root when cases use coded/dynamicCode objects."
        ) from exc
    return entry.pw_uid, entry.pw_gid


def _chown_case_tree(case_dir: str, user: str) -> None:
    uid, gid = _lookup_openfoam_run_user(user)
    for root, dirs, files in os.walk(case_dir):
...
def _run_as_user_args(user: str, command: List[str]) -> List[str]:
    ...
    su = shutil.which("su")
    if su:
        return [su, "-s", "/bin/bash", user, "-c", shlex.join(command)]

    raise RuntimeError(
        "Cannot drop OpenFOAM root privileges: neither 'runuser' nor 'su' is available."
    )


def openfoam_execution_args(channel: str, working_dir: str, *command: str) -> List[str]:
    """Build OpenFOAM execution args, dropping root privileges for local solver runs.

    OpenFOAM refuses to compile/load dynamicCode objects as root.  Some RheoTool
    tutorials use coded functionObjects, so local Foam-Agent runs must execute
    OpenFOAM commands as a normal user when the agent itself is running as root.
    """
    args = openfoam_command_args(channel, *command)
    if not _should_drop_openfoam_root_privileges(channel):
        return args

    user = _configured_openfoam_run_user()
    _chown_case_tree(working_dir, user)
    return _run_as_user_args(user, args)
...
def run_command(
    ...
    with open(out_file, 'w') as out, open(err_file, 'w') as err:
        try:
            command_args = openfoam_execution_args(
                channel,
                working_dir,
                "bash",
            ...
            message = (
                "FOAM FATAL ERROR: Foam-Agent OpenFOAM execution environment blocked\n"
                f"{exc}\n"
            )
            err.write(message)
            print(message.strip())
        ...
        try:
            ...
            if process.returncode != 0:
                err.write(f"\n[Foam-Agent] Allrun exited with status {process.returncode}.\n")
...
def check_foam_errors(directory: str) -> list:
    ...
    # DOTALL mode allows '.' to match newline characters. OpenFOAM may emit
    # either "ERROR:" or "FOAM FATAL IO ERROR:" depending on where it fails.
    pattern = re.compile(r"(?:FOAM FATAL(?: IO)? ERROR:|ERROR:)(.*)", re.DOTALL)
    files_to_check = {
        file for file in os.listdir(directory)
        if file.startswith("log") or file in {"Allrun.out", "Allrun.err"}
    }
    ...
    def _has_non_benign_error_word(text: str) -> bool:
        for line in text.splitlines():
            lowered = line.lower()
            if "error" not in lowered:
                continue
            if any(marker in lowered for marker in benign_error_markers):
                continue
            return True
        return False
    ...
    for file in sorted(files_to_check):
            ...
            elif "Allrun exited with status" in content:
                error_logs.append({"file": file, "error_content": content.strip()})
            elif _has_non_benign_error_word(content):
    ...
    if not error_logs and log_contents:
        ...
        for file, content in log_contents.items():
            rheotest_ramp_finished = (
                file == "log.rheoTestFoam"
                and "Your ramp is finished" in content
            )
            if rheotest_ramp_finished:
                continue
...
def retrieve_faiss(
    ...
    """Retrieve similar cases from a FAISS database.

    When metadata_filter is provided, the filter is a hard constraint: results
    whose metadata does not match are excluded and no cross-solver fallback is
    performed. We over-recall from FAISS before filtering to avoid small-solver
    starvation when the nearest global neighbours belong to other solvers.
    """

    if not isinstance(target, CaseTarget):
        raise TypeError("retrieve_faiss requires an immutable CaseTarget")

    faiss_cache = ensure_faiss_dbs_loaded()
    cache_key = f"{target.channel}/{database_name}"
    if cache_key not in faiss_cache:
        raise ValueError(f"Database '{cache_key}' is not loaded.")
    ...
    vectordb = faiss_cache[cache_key]
    search_k = int(topk)
    if metadata_filter:
        search_k = max(search_k, int(topk) * 20, 100)

    try:
        docs_and_scores = vectordb.similarity_search_with_score(query, k=search_k)
        docs = [d for d, _ in docs_and_scores]
        scores = [s for _, s in docs_and_scores]
    except Exception:
        docs = vectordb.similarity_search(query, k=search_k)
    ...
    if metadata_filter:
        filtered = [
            ...
            if _metadata_matches(doc.metadata or {}, metadata_filter)
        ]
        if len(filtered) < min_results:
            label = filter_label or f"{database_name}:{metadata_filter}"
            print(
                f"<rag_filter_starvation index=\"{database_name}\" label=\"{label}\" "
                f"returned=\"{len(filtered)}\" min_results=\"{min_results}\" "
                f"recall_k=\"{search_k}\">No cross-solver fallback was used.</rag_filter_starvation>"
            )
        docs = [d for d, _ in filtered[:topk]]
        scores = [s for _, s in filtered[:topk]]
    else:
        docs = docs[:topk]
        scores = scores[:topk]
    ...
    for doc, score in zip(docs, scores):
        ...
        elif database_name == "rheotool_manual_rules":
            formatted_results.append({
                "index": doc.page_content,
                "full_content": metadata.get("content", "unknown"),
                "source_type": metadata.get("source_type", "manual"),
                "manual": metadata.get("manual", "unknown"),
        ...
        elif database_name == "certified_benchmarks":
            formatted_results.append({
                "index": doc.page_content,
                "full_content": metadata.get("full_content", "unknown"),
                "source_type": metadata.get("source_type", "certified_benchmark"),
                "benchmark_id": metadata.get("benchmark_id", "unknown"),
                "benchmark_status": metadata.get("benchmark_status", "unknown"),
                "section": metadata.get("section", "unknown"),
                "topic": metadata.get("topic", "unknown"),
                "title": metadata.get("title", "unknown"),
                "case_set": metadata.get("case_set", "unknown"),
                "file_path": metadata.get("file_path", "unknown"),
                "source_doi": metadata.get("source_doi", "unknown"),
                "source_repo": metadata.get("source_repo", "unknown"),
                "source_case_path": metadata.get("source_case_path", "unknown"),
                "score": score,
            })
        else:
            raise ValueError(f"Unknown database name: {database_name}")

        formatted_results[-1].update({
            "channel": metadata.get("channel"),
            "version": metadata.get("version"),
            "constitutive_family": metadata.get("constitutive_family"),
            "model": metadata.get("model"),
            "phase_type": metadata.get("phase_type"),
            "geometry_family": metadata.get("geometry_family"),
            "flow_regime": metadata.get("flow_regime"),
