...
from utils import openfoam_command_args, save_file
from . import global_llm_service


def copy_custom_mesh(custom_mesh_path: str, user_requirement: str, case_dir: str, channel: str) -> Dict[str, Any]:
    """
    ...
    # Convert mesh
    try:
        result = subprocess.run(openfoam_command_args(channel, "gmshToFoam", "geometry.msh"), cwd=case_dir, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def run_checkmesh_and_correct(case_dir: str, python_file: str, max_loop: int, current_loop: int, channel: str) -> Tuple[bool, bool, str]:
    """Run checkMesh and optionally generate corrected code. Returns (success, should_continue, corrected_code)."""
    try:
        result = subprocess.run(openfoam_command_args(channel, "checkMesh"), cwd=case_dir, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    while gmsh_python_current_loop < max_loop:
        ...
        try:
            ...
            if os.path.exists(boundary_file):
                ...
                ok, should_continue, corrected = run_checkmesh_and_correct(case_dir, python_file, max_loop, gmsh_python_current_loop, channel)  # type: ignore
                if not ok:
