...
def _is_openfoam_environment_blocker(error_logs) -> bool:
    def _error_text(item) -> str:
        if isinstance(item, dict):
            return str(item.get("error_content", item))
        return str(item)

    text = "\n".join(_error_text(item) for item in (error_logs or []))
    return any(
        marker in text
        for marker in (
            "Foam-Agent OpenFOAM execution environment blocked",
            "This code should not be executed by someone with administrator rights",
...
def route_after_runner(state: GraphState):
    if state.get("error_logs") and len(state["error_logs"]) > 0:
        if _is_openfoam_environment_blocker(state["error_logs"]):
            print("<router>OpenFOAM execution environment blocked. Ending workflow without dictionary rewrite.</router>")
            state["termination_reason"] = "openfoam_environment_blocked"
            return END
        return "reviewer"
